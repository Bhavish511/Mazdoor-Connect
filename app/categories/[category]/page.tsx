...
import { updateFilter, clearFilters } from "@/store/slices/workersSlice";
...
import type { WorkerCategory, WorkerFilters } from "@/types";
import { Search, SlidersHorizontal, X } from "lucide-react";

export default function CategoryPage() {
  const params = useParams();
  const category = params.category as WorkerCategory;
  const { t, getWorkersByCategory } = useApp();
  const dispatch = useAppDispatch();
  const { filters } = useAppSelector((state) => state.workers);

  const [searchQuery, setSearchQuery] = useState("");
  const [localFilters, setLocalFilters] = useState<WorkerFilters>({
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      dispatch(
                        updateFilter({
                          sortBy: value as WorkerFilters["sortBy"],
                        })
                      )
                    }
                  >
